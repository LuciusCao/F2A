...
class F2A extends EventEmitter {
  constructor(options = {}) {
    ...
    // 如果是新生成的 ID，显示提示信息
    if (identity.isNew && identity.isAutoGenerated) {
      console.log('⚠️  未设置 F2A_AGENT_ID 环境变量');
      console.log(`🆔 已自动生成 Agent ID: ${this.myAgentId}`);
      console.log(`💾 身份已持久化保存到: ${identityManager.getConfigPath()}`);
      console.log('💡 如需自定义 ID，请设置环境变量:');
      console.log(`   export F2A_AGENT_ID="your-agent-name"`);
      console.log('');
    } else if (identity.isNew && !identity.isAutoGenerated) {
      console.log(`✅ 已设置 Agent ID: ${this.myAgentId}`);
      console.log(`💾 身份已持久化保存到: ${identityManager.getConfigPath()}`);
      console.log('');
    }
