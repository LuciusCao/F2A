...
F2A 使用 **UDP 多播 (Multicast)** 作为主要发现方式，**UDP 广播 (Broadcast)** 作为备用：

| 方式 | 地址/端口 | 说明 |
|------|-----------|------|
| 多播 (主) | `239.255.255.250:8768` | 主要发现方式，兼容旁路由/OpenClash 环境 |
| 广播 (备) | `<网段广播地址>:8767` | 每 15 秒发 1 次，作为备用 |

**为什么用多播？**
- 广播容易被旁路由/防火墙阻止
- 多播是"加入组"模式，更容易被路由器允许
- 兼容性更好，适合复杂网络环境
...
| 方式 | 地址/端口 | 说明 |
|------|-----------|------|
| 多播 (主) | `239.255.255.250:8768` | 主要发现方式，兼容旁路由/OpenClash |
| 广播 (备) | `<网段广播地址>:8767` | 每 15 秒发 1 次，作为备用 |
