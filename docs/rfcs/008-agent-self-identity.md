...
> - Phase 1 (核心组件): ✅ 完成
> - Phase 2 (CLI 改动): ✅ 完成
> - Phase 3 (Daemon 改动): ✅ 完成
> - Phase 4 (迁移与兼容): ⏭️ 不需要（无旧格式用户）
> - Phase 5 (测试与发布): ✅ 完成
...
└─────────────┘                              └─────────────┘
       ...
       │                                            │
       │  5. Store: CLI 保存身份文件                 │
       │  ~/.f2a/agent-identities/agent:fingerprint.json      │
...
# 输出: AgentId, 公钥指纹
# 注意: init 不设置 webhook，webhook 是运行时配置

# 注册到 Daemon（获取 Node 签名 + 设置 webhook）
f2a agent register --webhook "http://127.0.0.1:9002/webhook"
# 自动读取 init 生成的密钥对
# webhook 参数用于指定 Agent 的消息接收端点

# 更新 Agent 信息
f2a agent update --name "新名字"
# 仅用于修改 name，webhook 需通过 register 设置

# 查看身份状态
f2a agent status
# 输出: AgentId, 公钥, Node 签名状态, webhook 配置
...
│   └── 身份文件: ~/.f2a/agent-identities/agent:a3b2c1d4...json ← 私钥在此  │
│                                                                  │
│   Caller B (OpenClaw agent "猫咕噜")                             │
│   └── 配置: ~/.openclaw/f2a-identity.json                       │
│   └── 内容: { agentId: "agent:b7c8d9e0..." }                    │
│   └── 身份文件: ~/.f2a/agent-identities/agent:b7c8d9e0...json ← 私钥在此  │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
#    Identity file: ~/.f2a/agent-identities/agent:a3b2c1d4e5f67890.json
...
│                                                                  │
│  3. 读取身份文件                                                 │
│     ~/.f2a/agent-identities/agent:a3b2c1d4...json                         │
...
# → 身份文件: ~/.f2a/agent-identities/agent:a3b2c1d4...json
...
| Task | 文件 | 输出 |
|------|------|------|
| `f2a agent init` | `cli/agents.ts` | 密钥生成命令（不传 webhook） |
| `f2a agent register` | `cli/agents.ts` | 注册流程（含 --webhook 参数） |
| `f2a agent update` | `cli/agents.ts` | 更新 name 命令 |
...
### 为什么 webhook 不在 init 时设置？

1. **身份与运行时配置分离**：webhook 是 Agent 的运行时端点配置，而非身份固有属性
2. **支持多环境部署**：同一身份可在不同环境使用不同 webhook（开发、测试、生产）
3. **灵活的注册时机**：Agent 可能先在本地生成密钥，后续再部署并注册 webhook
4. **类比 SSH 模型**：SSH 密钥生成（`ssh-keygen`）不包含 authorized_keys，webhook 类似于"我在这"的注册

**设计决策**：
- `init`：生成密钥对，创建身份文件（无私钥、无私钥）
- `register`：向 Daemon 注册身份 + 设置 webhook 运行时端点
- `update`：修改 name 等非身份核心属性
...
2. 如果有 → 直接使用，跳过初始化
3. 如果没有：
   a. f2a agent init --name <name> --caller-config <path>
   b. f2a agent register --webhook <webhook-url>
4. 环境变量设置：export F2A_IDENTITY=<caller-config-path>
```

**说明**：
- `init` 只生成密钥对，不设置 webhook
- `register` 时传入 `--webhook` 参数设置运行时端点
- webhook 是运行时配置，不是身份属性
...
# CLI 从 ~/.f2a/agent-identities/{agentId}.json 读取 token
...
#   2. 从 ~/.f2a/agent-identities/{agentId}.json 读取 privateKey
