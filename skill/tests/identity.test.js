...
test('getOrCreateIdentity with preferred ID', () => {
  cleanup();
  const manager = new IdentityManager({
    configDir: tempDir,
    configFile: tempConfigFile
  ...
  const result = manager.getOrCreateIdentity('my-custom-id');
  
  assertEqual(result.agentId, 'my-custom-id', 'Should use preferred ID');
  assertTrue(result.isNew, 'Should be new');
  assertTrue(result.isPersistent, 'Should be persistent');
});

test('getOrCreateIdentity auto-generates ID when no preferred ID', () => {
  cleanup();
  const manager = new IdentityManager({
    configDir: tempDir,
    configFile: tempConfigFile
  ...
  assertTrue(result.isNew, 'Should be new');
  assertTrue(result.isPersistent, 'Should be persistent');
  assertTrue(result.isAutoGenerated, 'Should be marked as auto-generated');
});
...
test('getOrCreateIdentity loads existing identity', () => {
  ...
  const firstResult = manager1.getOrCreateIdentity('saved-id');
  ...
  const secondResult = manager2.getOrCreateIdentity();
  
  assertEqual(secondResult.agentId, 'saved-id', 'Should load saved ID');
...
test('identity is saved to config file', () => {
  ...
  manager.getOrCreateIdentity('test-save-id');
  ...
  assertEqual(saved.agentId, 'test-save-id', 'Saved ID should match');
...
test('config directory is created if not exists', () => {
  ...
  manager.getOrCreateIdentity('test-dir-id');
...
test('resetIdentity removes config file', () => {
  ...
  manager.getOrCreateIdentity('test-reset-id');
  assertTrue(fs.existsSync(tempConfigFile), 'Config file should exist');
...
test('hasPersistentIdentity returns true when config exists', () => {
  ...
  manager.getOrCreateIdentity('test-persist-id');
...
test('updating preferred ID overwrites existing', () => {
  cleanup();
  const manager = new IdentityManager({
    configDir: tempDir,
    configFile: tempConfigFile
  ...
  manager.getOrCreateIdentity('first-id');
  const result = manager.getOrCreateIdentity('second-id');
